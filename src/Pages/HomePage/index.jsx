...
import { useAwarnessMsg, usemodalmashora, useServicemain } from "../../Store";
...
export default function HomePage() {
  ...
  const {  openModalmashora } = usemodalmashora();

  const [Baby, setBaby] = useState([]);
  useEffect(() => {
    let copyfilterAwarness = [...allawarness].sort(
    ...
    <div className="parent flex-grow-1 d-flex " id={styles.parent}>
      <div className="col-12">
        <div className="" id={styles.imgparent}>
          ...
          <div
            className="col-12 text-center py-3 align-items-center justify-content-center"
            id={styles.sectionBaby}
          >
            <div
              className="container col-12 col-md-11 text-center d-flex align-items-center mt-4  justify-content-between gap-4 "
              dir="rtl"
              id={styles.sectionBabywrap}
            >
              {Baby.map((el, index) => (
                <Link
                  to={`/Services/${el.mashoraId}`}
                  key={el.mashoraId}
                  className=" text-white nav-link text-center d-flex flex-column align-items-center gap-3  justify-content-center"
                  data-aos="fade-up"
                  data-aos-offset="5"
                  data-aos-delay={`${index * 100}`}
                  id={styles.cardbaby}
                  onMouseEnter={() => setHoverimgMouse(index)}
                  onMouseLeave={() => setHoverimgMouse(null)}
                >
                  <img
                    src={hoverimgMouse == index ? el.hoverimg : el.img}
                    width={70}
                    height={70}
          ...
          <div
            className="col-12 container d-flex align-items-center"
            id={styles.msgmashora}
          >
            <div className="container col-12 d-flex justify-content-center align-items-center"    onClick={() => {
                        openModalmashora();
                      }}>
                ...
                {/* <h2>رسائل توعوية</h2> */}
                      ...
                      <div className="d-flex flex-column align-items-center">
                        <p className="fs-2">"يمكنك الآن حجز مشورة اون لاين والتواصل مع نخبة من الخبراء"</p>
                      </div>
        ...
        <NumberInitiative2 />
        <div
          className="col-12 container  d-flex justify-content-between align-items-center"
          id={styles.vedioandinfo}
        >
          <Video />
          <Infograph />
        </div>
