...
export default function Indicators() {
  ...
  const { vindicatorr, setvindicator, groupBy, setGroupBy } = usevindicator();
  ...
  const filtered = useMemo(() => {
    ...
    return vindicatorr.filter((item) => {
      const byIndicator = selectedIndicator
        ? item.indName === selectedIndicator
        : true;
      ...
      return (
        byIndicator && byService && byFrom && byTo && item.indValue !== null
      );
    });
      ...
      datasets: [
        {
          ...
          borderColor: labels.map((_, index) =>
            darkenHex(colorPalette[index % colorPalette.length])
          ),
          tension: 0.4,
          pointRadius: 5,
          pointHoverRadius: 6,
  ...
  const chartByDate = useMemo(() => {
    ...
    return {
      ...
      datasets: [
        {
          label: `${selectedIndicator} -  توزيع المؤشرات وفقاً للمحافظات `,
          data,
  ...
  return (
    <div>
      <div className="col-12 position-relative " id={styles.About}>
        <div className="col-12 " id={styles.AboutLogo}></div>
        <div
          className="col-12 position-absolute d-flex align-items-center "
          id={styles.bgColor}
        >
              ...
              <div className="col-12  " id={styles.regtangle}>
                <div className="container ">
                  <p className={styles.justifyText}>
                    يعرض هذا القسم مجموعة من المؤشرات توضح جهود المبادرة في كافة
                    محافظات الجمهورية. حيث تهدف المبادرة إلى تحسين صحة الأم
                    والطفل وأهمية الرضاعة الطبيعية، توعية الأسرة بضرورة المباعدة
                    بين الولادات، وخفض معدلات الولادات القيصرية، هذا بالإضافة
                    إلى تطوير مهارات الأم للاهتمام بالطفل في مراحل عمره
                    المختلفة.{" "}
                  </p>
                </div>
              </div>
      ...
      <div className="col-12">
        <div
          className="col-12 container d-flex flex-column p-4 rounded"
          style={{ direction: "rtl" }}
          id={styles.indecator}
        >
          <div
          ...
          >
            ...
            <select
              className={`px-5 py-1 me-md-5 me-2 rounded ${
                selectedIndicator === "" ? "text-secondary" : "text-dark"
              }`}
              value={typeService}
              onChange={(e) => handeltypeservicechange(e.target.value)}
            >
              <option className="text-secondary">اختار نوع الخدمة</option>
              {[...new Set(vindicatorr.map((i) => i.mashoraDesc))].map(
                (type, index) => (
            ...
            <span className={styles.customarrow2}>
              <IoMdArrowDropdown />
            </span>
          </div>
          <div
          ...
          >
            ...
            <select
              className={`px-5 py-1 me-md-5 me-2 rounded col-8 col-md-6 ${
                selectedIndicator === "" ? "text-secondary" : "text-dark"
              }`}
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <div className={`position-relative`}>
              <select
                className={`px-4 py-1 w-auto me-md-5 rounded ${
                  selectedIndicator === "" ? "text-secondary" : "text-dark"
                }`}
                value={fromYear}
              ...
              >
                <option>سنة</option>
                {[...new Set(vindicatorr.map((i) => i.indYear))].map(
                  (year, index) => (
            ...
            <div className={`position-relative`}>
              <select
                className={`px-4 py-1 w-auto rounded ${
                  selectedIndicator === "" ? "text-secondary" : "text-dark"
                }`}
                value={toYear}
          ...
          <div className="d-flex flex-column gap-3 mt-4">
            ...
            //   { name: "byGov", label: "توزيع المؤشرات وفقاً لتاريخ البيان" },
              { name: "byDate", label: "توزيع المؤشرات وفقاً للمحافظات" },
            ].map((item, idx) => (
              <div key={idx}>
                <label
                  className={`d-flex gap-2 align-items-center ${styles.purpleCheck}`}
                >
                  <input
                    type="checkbox"
          ...
          <div
            className="mb-3 mt-5 position-relative d-flex align-items-center col-12"
            style={{ maxWidth: "300px" }}
          >
            <label className="fw-bold d-block mb-2 col-6 ">
              نوع الرسم البياني:
            </label>
      ...
      <div className="container flex-lg-row flex-column-reverse mb-5 d-flex flex-wrap justify-content-between gap-4 align-items-center">
        {(typeService === "" || selectedIndicator === "", toYear === "") ? (
          ...
          <div className="col-12 text-center d-flex justify-content-center align-items-center">
            <p className="text-center fs-6 text-secondary">
              يرجى اختيار نوع الخدمة واسم المؤشر وخلال فترة لعرض الرسومات
              البيانية.
            </p>
          </div>
        ) : filtered.length === 0 ? (
          ...
          <>
            ...
            {filters.byDate && (
              <Chartscomponent2
                data={chartByDate}
                title={"  توزيع المؤشرات وفقاً للمحافظات "}
                selectedIndicator={selectedIndicator}
                type={chartType}
              />
            )}
