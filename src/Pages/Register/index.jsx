...
export default function Register() {
  ...
  const validationSchema = Yup.object({
    ...
    username: Yup.string()
      .required("الأسم ثلاثي مطلوب")
      .matches(
        /^\s*[\u0600-\u06FFa-zA-Z]+(\s+[\u0600-\u06FFa-zA-Z]+){2,}\s*$/,
        "الأسم يجب أن يكون ثلاثي على الأقل",
      ),
    name: Yup.string().required("اسم المستخدم مطلوب"),
    phone: Yup.string()
      .required("رقم الهاتف مطلوب")
      .matches(/^01[0-9]{9}$/, "رقم الهاتف يجب أن يكون 11 رقم ويبدأ بـ 01"),
    qualification: Yup.string().required("الحالة التعليمية مطلوبة"),
    statework: Yup.string().required("الحالة العملية مطلوبة"),
    password: Yup.string()
      .required("كلمة المرور مطلوبة")
      .min(6, "الباسورد لازم يكون 6 أحرف أو أكتر"),
    sSn: Yup.string()
      .required("الرقم القومي مطلوب")
      .matches(/^[23][0-9]{13}$/, "الرقم القومي غير صحيح"),
  });
  const hadleSubmit = (values, { resetForm }) => {
    postregisteration(domain, values)
      .then((res) => {
      ...
      <div className={`${styles.opinion}  p-md-5 py-5 mt-2 rounded`}>
        ...
        >
          <Form className="d-flex container flex-column gap-4" id={styles.form}>
            ...
            <div className="form-group text-end ">
              <div className="d-flex flex-column gap-2">
                <label htmlFor="phone">الرقم القومي</label>
                <Field
                  type="text"
                  dir="rtl"
                  name="sSn"
